...
from backend.schema.dashboard import DashboardSummaryResponse
...
# Create APIRouter instance for dashboard routes
...
async def get_dashboard_summary(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    # Querying similar data as for balances
    ...
    # Dashboard summary (you can add more fields like monthly spending, monthly income, etc. if required)
